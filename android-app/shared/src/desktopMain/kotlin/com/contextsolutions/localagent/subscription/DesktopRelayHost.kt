...
import com.contextsolutions.localagent.link.transport.DesktopRelayBytePipe
 ...
 * Ktor server uses ([com.contextsolutions.localagent.link.DesktopLinkRequestHandler]).
 *
...
class DesktopRelayHost(
    ...
    // shows "Mobile agent offline" + Disconnect instead of "No Local Agent paired yet".
        ...
        stateJob = scope.launch {
            ...
            p.state.collect { st ->
                ...
                if (st == LinkConnectionState.UP) {
                    ...
                    // so the status drops to "No Local Agent paired yet" (PR #90) and the next loop
    ...
    override suspend fun disconnect() {
        ...
        // below kills it) so the status drops to "No Local Agent paired yet" + hides Disconnect
