...
interface MemoryBackupOps {
    suspend fun export(destination: Uri): MemoryBackupController.ExportResult
    suspend fun import(source: Uri): MemoryBackupController.ImportResult
}
...
class MemoryBackupController @Inject constructor(
     ...
     */
    override suspend fun export(destination: Uri): ExportResult {
     ...
     */
    override suspend fun import(source: Uri): ImportResult {
        val raw = try {
            context.contentResolver.openInputStream(source)?.use { stream ->
                stream.readBytes().toString(Charsets.UTF_8)
            } ?: throw BackupException("Couldn't open the chosen file for reading.")
        } catch (t: Throwable) {
            if (t is BackupException) throw t
            Log.w(TAG, "import read failed: ${t.message}")
        ...
        for (entry in export.memories) {
            ...
            val embedding = try {
                ...
                Log.w(TAG, "embed failed during import: ${t.message}")
                null
            }
            ...
            try {
                ...
                Log.w(TAG, "insert failed during import: ${t.message}")
                skipped += 1
            }
        }

        counters.increment(CounterNames.MEMORY_IMPORTED_TOTAL)
        Log.i(TAG, "imported=$imported skipped=$skipped replaced=$previousCount")
        return ImportResult(
    ...
    private companion object {
        ...
        /** Tracked here rather than in :shared because the embedder model id is an Android-asset concern. */
