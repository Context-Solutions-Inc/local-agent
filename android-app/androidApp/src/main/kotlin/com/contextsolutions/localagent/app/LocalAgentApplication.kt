...
import com.contextsolutions.localagent.app.service.TelemetryUploadWorker
import com.contextsolutions.localagent.app.service.UnloadReason
import com.contextsolutions.localagent.app.di.androidModule
...
class LocalAgentApplication : Application() {
    ...
    override fun onCreate() {
        ...
        if (GlobalContext.getOrNull() == null) {
            startKoin {
                androidContext(this@LocalAgentApplication)
        ...
        //
        // Guarded on Firebase having auto-initialized: `google-services.json` is
        // gitignored, so it's absent in CI / fresh checkouts and when its
        // `package_name` doesn't match the `applicationId`. When it's missing,
        // FirebaseApp never initializes and `FirebaseAnalytics.getInstance()` throws
        // — skip the analytics wiring (the crash reporter is already NoOp, see
        // androidModule) rather than crash on launch.
