 ...
 * then call `MobileAgentDatabase.Schema.migrate(driver, 1, 2)` — the same
...
class MemoriesMigrationTest {
    ...
    fun schema_version_reflects_all_sqm_migrations() {
        ...
        // the desktop port's Phase 7 added 7.sqm (tasks table), and PR#57 added
        // 8.sqm (conversations/memories soft-delete tombstones + memory updated_at
        // for sync) — current version must be 9. If this fails after a new schema
        // change, either the SQLDelight build didn't regenerate the schema or the
        // new .sqm is misnamed.
        assertEquals(9L, MobileAgentDatabase.Schema.version)
    }
    ...
    fun migrate_one_to_two_adds_access_count_column_with_default_zero() {
        ...
        // upgrade detection.
        val result = MobileAgentDatabase.Schema.migrate(driver, oldVersion = 1, newVersion = 2)
    ...
    fun migrated_table_accepts_inserts_with_explicit_access_count() {
        createV1MemoriesTable(driver)
        MobileAgentDatabase.Schema.migrate(driver, oldVersion = 1, newVersion = 2)
    ...
    fun migrate_v1_to_v3_chains_through_all_sqm_files() {
        ...
        val result = MobileAgentDatabase.Schema.migrate(driver, oldVersion = 1, newVersion = 3)
        assertEquals(QueryResult.Unit, result)
