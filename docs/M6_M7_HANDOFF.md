...
| B — Eager Gemma load + thermal infra | ✅ host-side complete 2026-05-10 | First-token on Pixel 7 measured ~1–3s subjectively (cold-open-then-send), well under the 5s upper bound. **Target revised from <1.5s to 1–5s acceptable, <5s required** (see §5 Phase G finding 2). `loaded(GPU)` warm-up line confirmed post-launch. |
