...
- **`secrets.properties`** in `android-app/` (NOT the repo root — see `secrets.properties.example`).
  ...
  (`models.contextsolutions.com`, PR #28) with no auth** (PR #22 — URLs + checksums are pinned in `ModelInventory`/`AndroidAuxModels`, so the old
...
mkdir -p models
base=https://models.contextsolutions.com
