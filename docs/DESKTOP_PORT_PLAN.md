# Desktop port of Local Agent — Compose Multiplatform + llama.cpp (v0.1.0)
...
2. Platform Koin modules (`androidMain`/`desktopMain`) provide actuals: `InferenceEngine`, `ClassifierEngine`, `EmbedderEngine`, `HttpEngineFactory`, `SecureStorage`, `SqlDriver`/`LocalAgentDatabase`, all preferences repos, `AlarmScheduler`, `AgentClock`, `LocaleProvider`, headroom/thermal providers, telemetry sinks.
...
- **As-built (PR #94): aux-model auto-download.** `DesktopAuxModelStore` now fetches the two `.onnx` into app-data `models/` on first run (mirroring the GGUF/Vosk path) via `DesktopModelDownloader`, with sha256 + size pinned in `DesktopAuxModels`; `Main.kt` ensures them before `warmUp()`. The hosting endpoint is **compile-time-configurable** — `-PauxModelBaseUrl=https://host/path` is baked into `desktop_build_info.properties` and read by `DesktopAuxModels.baseUrl()`; the default placeholder is treated as unconfigured, so the store skips the fetch (no doomed network call) and falls back to a manually-placed file or the `LOCALAGENT_{CLASSIFIER,EMBEDDER}_ONNX` env override. See CLAUDE.md invariant #61.
...
| SQLDelight driver | `sqldelight.sqlite.driver` (JDBC) → file in app data dir; `LocalAgentDatabase` factory unchanged |
...
- Android shell shrinks to: `MainActivity`, `LocalAgentApplication` (Koin start), Android Koin module, Android-only services (clock receivers, foreground service), Android actuals.
...
- New `:desktopHarness` (kotlin-jvm + application) wires the **real** `AgentLoop` + `PromptAssembler` + `PreflightRouter` + `SearchService` (search disabled) with an in-memory `LocalAgentDatabase` via the JDBC SQLite driver — also validating the SQLDelight JVM-driver seam. `Main.kt` runs a chat turn; `EngineSmoke.kt` is a low-token native-path probe.
...
- **DatabaseModule** (commit 8d4f693): `LocalAgentDatabase` (AndroidSqliteDriver, same `local_agent.db`) + the 4 query handles → `androidModule`; query handles resolve by type. `ConversationRepository` stays Hilt-constructed over Koin-backed queries.
...
**Increment 2 — `AgentLoopFactory` → commonMain (`a3579bd`):** interface + Koin binding moved to `:shared` `com.contextsolutions.localagent.di` (`agentCoreModule`); required collaborators via `get()`, optional ones via `getOrNull()` so the search/memory-disabled desktop graph still builds a loop. New `AgentLogger` fun interface keeps the `"AgentLoop"` tag (#28) per-platform. Pinned desktop `kotlinx-datetime` to 0.6.1 (Compose Material3 1.9.0 drags in 0.7.x which relocated `kotlinx.datetime.Clock`).
...
- Both callsites flipped `hiltViewModel()` → `koinViewModel()`: `TodoManagementScreen.kt:74`, `ChatScreen.kt:127` (other VMs on that screen stay on Hilt — coexistence is fine; `koinViewModel()` resolves via the Koin GlobalContext started in `LocalAgentApplication.onCreate`).
...
- `DesktopAuxModels` (desktopMain `inference/`) resolves the two `.onnx` paths from the app-data `models/` dir (mirroring the GGUF) or a `LOCALAGENT_{CLASSIFIER,EMBEDDER}_ONNX` absolute-path env override — the "simple path/env" the plan sanctions before the full Phase-6 download/`ResourceLoader` wiring.
...
- **CI-skip contract:** each test no-ops with a printed `SKIP` when its `.onnx` is absent. Operator runs it by placing the `.onnx` in the app-data `models/` dir or setting `LOCALAGENT_{CLASSIFIER,EMBEDDER}_ONNX`.
...
cd ../android-app
LOCALAGENT_CLASSIFIER_ONNX=$PWD/../models/preflight_memory_shared_v1.0.0.onnx \
LOCALAGENT_EMBEDDER_ONNX=$PWD/../models/all-MiniLM-L6-v2.onnx \
...
**Operator note (search/memory on desktop):** drop the exported `.onnx` models in the app-data `models/` dir (or set `LOCALAGENT_{CLASSIFIER,EMBEDDER}_ONNX`), set a Brave key via `BRAVE_API_KEY` (or `SecureStorage`), and the GGUF — then the desktop agent runs the full pre-flight → vertical-search → memory loop. App-data dir: `~/.local/share/LocalAgent/` (Linux), `~/Library/Application Support/LocalAgent/` (macOS), `%LOCALAPPDATA%\LocalAgent\` (Windows).
...
- **Verified on this host:** `:desktopApp:createDistributable` produces a complete app-image — `bin/LocalAgent` launcher, bundled `lib/LocalAgent.png`, the `llama`/`onnxruntime`/`vosk`/`sqlite-jdbc` native-bearing jars on the classpath, a 50-entry runtime image, and `-Xmx2g` / `-Dfile.encoding=UTF-8` in `LocalAgent.cfg`. `:desktopApp:packageDeb` reaches the DEB bundler and stops **only** on missing `fakeroot` (the `.deb` wrap = operator/CI). `:desktopApp:compileKotlin` + `DI_CHECK=1 :desktopApp:run` (`opened schema v8`, "resolved OK") + **`:androidApp:assembleDebug`** (cardinal) all green.
...
- **Callsites:** the two importers (`MainScreen` → `TodoManagementScreen`, `ChatScreen` → `TodoViewModel` for the header badge) re-pointed to `com.contextsolutions.localagent.ui.todo`. `koinViewModel()` callsites unchanged (resolution is package-agnostic).
...
- **`AppBuildConfig` seam (NEW commonMain interface, `platform/`):** `isDebug`/`isInternalBuild`/`hasBraveDevKey`/`hasHfDevToken`. `SettingsViewModel` referenced `:androidApp`'s generated `BuildConfig` (dev keys + DEBUG), invisible from `:ui`. Android actual `AndroidAppBuildConfig` (in `:androidApp`, reads `BuildConfig`, bound in `androidModule`); desktop `DesktopAppBuildConfig` (constant — no bundled dev secrets; `isDebug` from `-Dlocalagent.debug`, bound in `desktopModule`). The VM exposes `isDebugBuild` through `SettingsUiState` so the screen's `BuildConfig.DEBUG` gate became `state.isDebugBuild`.
