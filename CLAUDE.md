...
14. **Pre-flight thresholds (0.85 high / 0.15 low) are CONFIGURABLE per PRD §3.2.1.** Shipped via `:androidApp/src/main/assets/preflight_config.json`, read at startup. Don't hard-code — that blocks telemetry-driven tuning, which is the documented path to closing the v1.0 §7 precision gap.
