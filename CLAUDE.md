...
- **Architecture:** KMP shared module (`:shared`) + Android Compose shell (`:androidApp`). iOS stubbed for Phase 2.
- **Toolchain:** JDK 17, Gradle 9.3.1, AGP 9.1.1, Kotlin 2.3.21, KSP 2.3.7, Hilt 2.59.2.
- **Status:** M0–M6 complete (`PHASE1_PLAN.md` §5). M7 (closed beta → Play Store) not started.
...
40. **LiteRT-LM and the classifier runtime ship colliding `libLiteRt.so` — pin BOTH to `litert:2.1.5` + `pickFirst`, never bump independently.** LiteRT-LM ≤0.10.2 statically linked its core (isolated — the basis of #18); 0.11.0+ ships standalone `libLiteRt.so` + `libLiteRtClGlAccelerator.so` sharing a SONAME with `com.google.ai.edge.litert`'s libs. Android packages one `.so` per name and the two builds are never byte-identical — resolved via `packaging { jniLibs { pickFirsts += ["**/libLiteRt.so", "**/libLiteRtClGlAccelerator.so"] } }` in `androidApp/build.gradle.kts`. **They are NOT interchangeable:** (a) LiteRT-LM's `libLiteRt.so` lacks the `Java_com_google_ai_edge_litert_*` JNI symbols → forcing it crashes the classifier with `UnsatisfiedLinkError: Environment.nativeCreate`; (b) `litert:2.1.4`'s libs SIGSEGV `liblitertlm_jni.so` 0.12.0 in GPU init. **`litert:2.1.5` is the one combination that serves both** (`pickFirst` lands on litert's copies, alphabetically first). So `litertlm` and `aiEdgeLitert` versions are **coupled** — bumping LiteRT-LM means re-checking the litert version satisfies both the classifier and the LLM on-device.
