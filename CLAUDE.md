...
- **Remote LLM (both platforms, optional):** user can point the *chat LLM* at a LAN **Ollama** server (Settings → Ollama server). `RoutingInferenceEngine` (commonMain) wraps the local engine, routes to `OllamaInferenceEngine` (pure-Ktor, OpenAI-compatible, text + image) when configured + reachable, else local. Classifier/embedder/search/memory always stay on-device. See #44. (Ollama-only; OpenRouter/generic OpenAI-compatible not supported.)
...
29. **`Conversation.cancelProcess()` is the ONLY way to stop an in-flight decode.** `Job.cancel()` closes the Flow but the native worker decodes to end-of-turn, holding GPU/CPU and freezing the UI. `LiteRtInferenceEngine.bindCancellation` fires `cancelProcess()` on parent cancel (finally block, before `conversation.close()`). The watchdog drives it: `InferenceSessionManager.forceUnload(UnloadReason.MainThreadWatchdog)`. **`forceUnload` cancels in-flight generations ONLY for `MainThreadWatchdog`**; `LowMemory`/`TrimMemory`/`Manual` defer (cutting a live turn is worse UX). Best-effort if the GPU is wedged so hard `cancelProcess()` can't return. (Not in public docs.)
44. **The remote-Ollama path lives entirely behind the `InferenceEngine` seam — never special-case it above the seam.** `RoutingInferenceEngine` is `single<InferenceEngine>` on both platforms; decides backend **once per `loadModel`** (configured+reachable→Ollama, configured+unreachable→local, unconfigured→local), tags the `RoutedHandle`. Sits **below** Android `InferenceSessionManager` / desktop `WarmModel`, so agent loop, session lifecycle, service, watchdogs are unchanged. **Two things drop the resident handle so the next turn re-decides:** (a) Settings change — owners observe `OllamaPreferences.configFlow()`; (b) server offline/online transition — `OllamaConnectionMonitor`. `OllamaInferenceEngine` calls the monitor on a **connection** failure (NOT an HTTP-status error) and on successful connect; monitor emits `reloadRequests` (→ `forceUnload(Manual)` / `WarmModel.invalidate()`) + runs a ~15 s health-watch to auto-reconnect. **`Accelerator.REMOTE`** is reported by the Ollama handle through `RoutedHandle`; `SessionBanner` renders nothing for `Loaded(REMOTE)`; `LiteRtInferenceEngine.backendFor` rejects it. **Image turns** use the configured vision model else the chat model (no cross-backend switch mid-handle). Streaming client must be **un-timed** — `OllamaInferenceEngine.loadModel` rebuilds it with `requestTimeoutMillis = null` (the factory's 10 s timeout aborts long streams). Android needs cleartext (`network_security_config.xml`) for `http://` LAN servers.
...
18. **Pre-flight classifier MUST run on `com.google.ai.edge.litert:litert:2.x` — NOT `org.tensorflow:tensorflow-lite`, NOT Play Services TFLite.** Both classic runtimes silently produce broken output for our `ai-edge-quantizer` INT8 model (logits ~1500× reference, every query collapses to one class, no exception). `litert:2.x` is the Android port of Python's `ai-edge-litert` (package `com.google.ai.edge.litert.CompiledModel`, different native libs). LiteRT-LM keeps its own runtime via `play-services-tflite-*`; both coexist. **Class-collision workaround:** litert AAR + transitive `tensorflow-lite-api` both bundle `org.tensorflow.lite.*` — exclude `org.tensorflow:tensorflow-lite-api` via `configurations.matching {…}.configureEach { exclude(...) }` in both `:shared` and `:androidApp`.
...
42. **Voice I/O is entirely `:androidApp` (no `:shared` seam), behind `ChatSpeaker`/`SpeechDictation`.** Input row order: **mic · speaker · photo · Send**.
    ...
    - **Dictation streams the live transcript into the prompt box (PR #67) — partials are display-only; commands + committed text key off finalized results.** `Dictation` exposes `partials: Flow<String>` (live, rewrites mid-utterance) alongside `results: Flow<String>` (finalized utterances). Android sets `EXTRA_PARTIAL_RESULTS=true` + emits in `onPartialResults`; desktop Vosk emits `recognizer.partialResult`'s `"partial"` field per audio frame (deduped). `ChatScreen` keeps a `committedInput` anchor + renders `committedInput (+ space) + partial`; the finalized `results` emission (NONE branch) **or a manual keystroke** promotes the partial into `committedInput`. **`VoiceCommand.match` runs ONLY on `results`** (whole utterances) — never on a partial — so send/cancel/etc. fire as before; the transient command partial flashing in the box is discarded on finalize. Echo suppression drops partials during TTS playback too. **Don't act on a partial** and **don't send `input`** for a voice SEND — send `committedInput`.
...
48. **Desktop read-aloud (PR #66) shells out per-OS (`say`/`spd-say --wait`/PowerShell `SpeechSynthesizer`) — and stopping it is OS-specific.** `DesktopTtsSpeaker.stop()` calls `process.destroy()`, which silences macOS `say` + the Windows synthesizer (in-process) but **NOT Linux**: `spd-say` is only a *client* handing text to the speech-dispatcher daemon, which keeps playing after the client dies. So `stop()` **also issues `spd-say --cancel`** on Linux — that's what makes the speaker-mute toggle silence in-flight speech there. **Desktop voice IS configurable** (unlike Android): engine (Linux output module, `spd-say -o`) / voice (`-y` / `say -v` / `SelectVoice`) / rate live on the **concrete `DesktopTtsPreferences`** (`tts_prefs.json`), deliberately OFF the shared `TtsPreferences` interface (same split as desktop UI-zoom, #45). `DesktopTtsVoices` enumerates options (`spd-say -O`/`-L`, `say -v ?`, PowerShell `GetInstalledVoices`); the Settings picker is a desktop-only `expect/actual DesktopVoiceSection()` (Android actual = no-op). **Real quality win is the bundled Piper neural engine (PR #66), not the OS engine.** `engine == DesktopVoiceConfig.PIPER_ENGINE` ("piper") routes `DesktopTtsSpeaker` to `PiperSpeechSynthesizer`. Piper is self-contained + offline: `PiperBinaryStore` downloads the prebuilt `rhasspy/piper` executable (pinned `PiperRelease.TAG`, per-OS/arch asset table w/ verified sha256, extract via system `tar` / Windows `ZipInputStream`, `$ORIGIN`-rpath bundles onnxruntime+espeak-ng-data) and `PiperVoiceStore` downloads the ONNX voice + `.onnx.json` (HF `rhasspy/piper-voices`, `en_US-lessac-medium` ≈ 63 MB) — both via the same `DesktopModelDownloader`/`DesktopModelInventory`/app-data layout as the LLM/Vosk models. Synth is `piper --model … --config … --output-raw` (raw S16LE mono @ model sample rate) streamed to a **`javax.sound.sampled.SourceDataLine`** — no `aplay`/Python. So Piper **stops instantly** (in-JVM line close + process kill; the daemon-cancel problem is Linux-`spd-say`-only). Rate maps to `--length_scale` (smaller = faster). The picker offers Piper only when `PiperRelease.assetForHost() != null`; selecting it triggers `prepare()` (pre-download w/ progress in `PiperState`). OS speech-dispatcher modules remain the non-neural fallback.

### Desktop appearance & window (CMP, PR #61)

45. **Desktop UI "zoom" scales `LocalDensity.density`, NOT `fontScale` — and they're different levers.** `AppThemeScaffold` (shared `:ui`) takes `densityScale: Float = 1f` (default ⇒ Android untouched) and applies `Density(base.density * densityScale, base.fontScale * fontScale)`. Density scales `dp`+`sp` (whole UI: icons, padding, forms, text); `fontScale` is the text-only Settings slider. **For "everything too small" use density, never fontScale.** Desktop `Window(onKeyEvent=…)` drives it via **Ctrl/Cmd `+`/`-`/`0`** (modifier-gated so it never eats typed chars); the level is persisted desktop-only on `DesktopThemePreferences.uiZoom*` (`theme_prefs.json`, `UiZoom` bounds 0.8–2.0) — kept OFF the shared `ThemePreferences` interface. `Main.kt` reads the **concrete** `DesktopThemePreferences` from Koin (the zoom methods aren't on the interface).
46. **Desktop theme is monochrome, not the Android green; `ThemeMode.System`/Auto is unreliable on Linux.** Desktop `Theme.kt` defines `LightMonochromeColorScheme` (white surfaces, black trim) + `DarkMonochromeColorScheme` (near-black `#121212`, white trim), with `surfaceTint = Color.Transparent` to kill M3 tonal-elevation overlays; `error` stays red. Android keeps Material You / the green fallback (desktop-only divergence). **`isSystemInDarkTheme()` on desktop is backed by Skiko — works on macOS/Windows, but on Linux it commonly reports light with no live update (JetBrains CMP-6028).** No Linux detector wired; use the manual Light/Dark selector there. Status dots (memory/connection green-yellow-red in `:ui`) are semantic, not theming — leave them.
47. **Desktop window geometry is persisted; chat bubbles size to window width.** `DesktopWindowPreferences` (`window_prefs.json`) saves size/position/maximized; `Main.kt` seeds `rememberWindowState` from it + saves on change via `snapshotFlow` + `collectLatest` debounce (width/height saved only while *floating* so maximizing doesn't clobber the restore size; position only when absolute). Chat bubbles: `ChatScreen.kt` wraps the message list in `BoxWithConstraints` — `<600.dp` keeps the mobile-tuned 320 dp cap, wider uses `(maxWidth*0.72f).coerceIn(480.dp,760.dp)`; threaded as `maxWidth: Dp` into `UserBubble`/`AssistantBubble`/`StreamingAssistantBubble`.

### Jobs (PR #70)

Time-triggered work that runs **only on the desktop agent** (cron or one-shot), with a mobile remote view. Full as-built design lives in `docs/JOBS_PLAN.md` §0.

49. **A job is `command` + `prompt`, always a desktop subprocess; each run continues the job's ONE conversation thread.** No COMMAND-vs-AGENT_PROMPT split, no `TaskQueue` path. `JobExecutor` runs `command` with `prompt` as a **bound positional shell arg** (`sh -c '<cmd> "$1"' sh "<prompt>"` — injection-safe; PowerShell on Windows), then appends `user = prompt` / `assistant = response` (`renderMarkdown = false`) to the job's existing conversation (`last_run_conversation_id`), creating one only if absent — so re-running a job (incl. a re-scheduled one-shot) accumulates history in a single thread. That conversation syncs via the normal `ConversationSyncRecord` path; "View conversation" opens it through `ChatViewModel.loadConversation`. `JobScheduler`/`JobExecutor`/`JobService` (impl the commonMain `JobAdmin` seam; **null on mobile → read-only UI**) are desktopMain; `cron-utils` is **desktopMain-only** (the synced model carries only the `cronExpression` string).
50. **Job trust boundary = the injected `JobSyncPolicy`, fail-closed; the `*FromPeer` write needs the `onJobPausedFromPeer` seam to re-drive the scheduler.** Create/edit/delete/run-now are **desktop-only**; a mobile peer may only toggle `paused`. `DesktopJobSyncPolicy` drops remote inserts + remote tombstones and applies paused-only to an **existing** row; `MobileJobSyncPolicy` applies authoritative desktop records verbatim. Both run inside the shared `SqlDelightLinkSyncService` (bound per-platform). The peer-pause path writes the **raw** `updatePausedFromPeer` query (so it fires no `LocalChangeBus` → no echo), which means the desktop scheduler won't react unless `applyFromPeer` calls the desktop-bound `onJobPausedFromPeer` callback → `JobService.reactToPausedChange` (cancel/re-arm). `jobs` sync LWW + tombstone with the **denormalized** last-run (`last_run_status/at/summary` + `last_run_conversation_id`) on the row; `job_runs` is **desktop-local history, NOT synced**.
51. **`SqlDelightJobRepository.flow()` MUST be a reactive SQLDelight query (`selectAllJobs().asFlow().mapToList`), NOT a seeded `StateFlow`.** Synced rows are written by the sync layer via the raw `*FromPeer` queries that **bypass the repo**, so the tasks/todos "MutableStateFlow republished on local mutation" pattern goes stale → a desktop job synced to the phone is invisible until an app restart. The reactive query repaints on any write to the `jobs` table on that driver (both directions). Local writes still call `LocalChangeBus.notifyChanged()` for the sync push.
52. **No Material3 `DatePicker` on desktop — its kotlinx-datetime calendar model throws `NoSuchMethodError` (`KotlinxDatetimeCalendarModel.getToday`).** The Jobs schedule form reuses the mobile clock UI: **Repeat** = the alarm flow (`TimePicker` time + day-of-week chips → 5-field cron `min hour * * dows`, `*` = daily), **Once** = the timer flow (h/m/s duration → one-shot `fireAt = now + duration`). Desktop swaps the analog clock for **`TimeInput`** (text fields) — the clock's selector knob obscured the selected digit under the monochrome theme. That theme also had to define the Material3 `surfaceContainer*` roles (`Theme.kt`); unset, they fall back to M3's purple baseline and tint `AlertDialog`/menus light purple. Mobile `RuleSettingsIcon` (Material Symbol `rule_settings`) is a hand-built `ImageVector` — the pinned material-icons-extended predates the symbol.
